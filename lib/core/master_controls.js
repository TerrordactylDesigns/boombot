...
exports.script = function(boombot, text, uname, uid, private) {
  ...
  if (text.match(/uptime/i)) {
    ...
    boombot.respond(uid, 'Current uptime is: ' + upTime, private)
  }
  /*
    AUTOVOTE
  */
  ...
  if (text.match(/reload/i)) {
    ...
    // TODO
    // make this better.....
    boombot.commands = require('../load').LoadCore(boombot.commands)
    boombot.commands = require('../load').LoadQueue(boombot.commands)
    boombot.commands = require('../load').LoadOptional(boombot.commands)
  }
  /*
    PM Speech
  */
