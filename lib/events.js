...
exports.roomChangedEvent = function(boombot, data){
  ...
  var users   = data.users
    , currDj  = data.room.metadata.current_song.djid
  ...
  if (data.room.metadata.votelog[0].toString().match(/down/i)) {
    try {
      var uncut = data.room.metadata.votelog[0].toString()
        , chopped = uncut.substring(0, uncut.indexOf(','))
  ...
  */
  if (boombot.config.lastfm.use && boombot.config.lastfm.scrobble && data.room.metadata.current_song.metadata.length > 30) {
    var scribble  = require('scribble')
      , scrobbler = new scribble(boombot.config.lastfm.API_key, boombot.config.lastfm.secret, boombot.config.lastfm.username, boombot.config.lastfm.password)
      , song      = {artist:currArtist, track:currSong}
    ...
    */
    if ((uid == boombot.config.admin.userid
      ...
      /*
        MOD GROUP CHAT
      */
      if (private && !ran && modArray.contains(uid) && boombot.config.moderatorChat && boombot.theUsersList[uid]) {
