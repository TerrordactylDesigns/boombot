...
exports.registeredEvent = function(boombot, data) {
  ...
  if (boombot.shutUp == false) {
    ...
    //theme private messaging - Makes the bot pm user if a theme has been set.
    if('theme' in boombot && boombot.theme !== null)
    boombot.bot.pm('Current theme is: ' + boombot.theme, data.user[0].userid, function(data) { })
  }
}
...
exports.newsongEvent = function(boombot, data) {
  ...
  boombot.bot.getFans(data.room.metadata.current_dj, function(data2) {
    boombot.theUsersList[data.room.metadata.current_dj].startFans = data2.fans.length
  })
  //auto bop
  if (boombot.autoNod) {
    setTimeout(function(){
...
exports.endsongEvent = function(boombot, data) {
  ...
  if (boombot.config.stats && boombot.shutUp == false) {
    var snags = boombot.snagCounter
    boombot.bot.getFans(currDj, function(data2) {
      boombot.bot.speak(data.room.metadata.current_song.metadata.song + " by " + data.room.metadata.current_song.metadata.artist + " got :+1: " + data.room.metadata.upvotes + " :-1: " +  data.room.metadata.downvotes + " <3 " + snags + " :star: " + (data2.fans.length - boombot.theUsersList[currDj].startFans))
    })
  }
  if (boombot.queue)
    if (boombot.theUsersList[currDj].plays >= boombot.queueLength)
      boombot.bot.remDj(currDj)
  if (boombot.scrobble)
