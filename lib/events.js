...
exports.endsongEvent = function(boombot, data) {
  ...
  if (boombot.config.stats && boombot.shutUp == false) {
    boombot.bot.getFans(currDj, function(data2) {
      boombot.bot.speak(data.room.metadata.current_song.metadata.song + " by " + data.room.metadata.current_song.metadata.artist + " got :+1: " + data.room.metadata.upvotes + " :-1: " +  data.room.metadata.downvotes + " <3 " + boombot.snagCounter + " :star: " + (data2.fans.length - boombot.theUsersList[currDj].startFans))
    })
  }
  if (boombot.queue)
