...
exports.registeredEvent = function(boombot, data) {
  ...
  if (boombot.shutUp == false) {
	...
	/**///theme private messaging angryrancor 4/13/13
	//Makes the bot pm user if a theme has been set.
	
	//Checks if we have modified models\robot.js as well with variables.  If not, does nothing. 
	if( boombot.theme && boombot.noTheme
	     //Check for if theme set since bot was started.  If theme not set don't private message user.
	     && boombot.theme != boombot.noTheme
	   )
		  //Send a private message to user
		  boombot.bot.pm('Current theme is: ' + boombot.theme, data.user[0].userid, function(data) { })
	/**///end theme private messaging
  }
}
